...
describe("POST /users", function () {
  ...
  test("works for admins: create admins", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("unauth for non-admins and incorrectusers", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toBe(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("works: set new password by correctuser", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
